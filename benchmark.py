...
def extract_multiple_ply_files(ply_folder, output_bin_path, max_files_to_sample=3):
    ...
    if os.path.exists(output_bin_path):
        print(f"Target production binary already exists at {output_bin_path}. Skipping extraction.")
        return

    ply_files = glob.glob(os.path.join(ply_folder, "*.ply"))
    if not ply_files:
        raise FileNotFoundError(f"No .ply files found inside the target folder: {ply_folder}")
    ...
    with open(output_bin_path, "wb") as bin_file:
        for idx, ply_path in enumerate(files_to_process):
            print(f"[{idx+1}/{len(files_to_process)}] Parsing 'testing' elements from {os.path.basename(ply_path)}...")
            try:
                ...
                matrix_block = np.stack((X, Y, Z, intensity, sem_class, ins_class), axis=1)
...
def run_spatial_streamio_pipeline(file_path, batch_size, model):
    ...
    while True:
        ...
        if batch is None:
            print("   -> Epoch end signaling token received safely. Terminating epoch processing run.")
            break
...
if __name__ == "__main__":
    ...
    try:
        extract_multiple_ply_files(training_ply_folder, target_bin_file, max_files_to_sample=3)
    ...
    pipeline_dur = run_spatial_streamio_pipeline(target_bin_file, BATCH_SIZE, spatial_model)
