        ...
        while not self.stop_event.is_set():
            if self.current_step >= len(self.indices):
                ...
                try:
                    ...
                    self.current_step = (
                        0  # Reset index step for the next potential epoch
                    )
                    break
                except queue.Full:
            ...
            try:
                ...
                print(
                    "Pipeline prefetch worker timed out waiting for an open queue slot."
                )
                time.sleep(0.1)
    ...
    def next_batch(self):
        ...
        if not self.running:
            raise RuntimeWarning(
                "Pipeline engine is not currently running. Call start() first."
            )
        try:
            return self.queue.get(block=True, timeout=self.timeout)
        except queue.Empty:
            raise TimeoutError(
                "Data streaming starved: Prefetch thread failed to load buffers within timeline windows."
            )
