...
class MemoryMappedSpatialReader:
    ...
    def __init__(self, file_path, num_features=4, dtype=np.float32):
        """
        Parameters:
        - file_path: Path to the raw uncompressed binary file (.npy or raw bytes)
        - num_features: Number of elements per point (e.g., 4 for X, Y, Z, Class_ID)
    ...
    def get_slice(self, start_idx, end_idx):
        ...
        if start_idx < 0 or end_idx > self.num_points:
            raise IndexError(f"Slice range [{start_idx}:{end_idx}] exceeds data bounds [0:{self.num_points}].")
    ...
    def close(self):
        ...
        if hasattr(self, 'mmap_array'):
            del self.mmap_array# Spatial-StreamIO Memory Mapping Module 
